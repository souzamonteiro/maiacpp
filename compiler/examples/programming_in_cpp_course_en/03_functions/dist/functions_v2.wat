;; Generated by MaiaC
;; Template backend: human-readable WAT
;; Style inspired by maiawasm/assembler/tests/fixtures

(module
  (import "env" "__exc_push" (func $imp___exc_push))
  (import "env" "__exc_pop" (func $imp___exc_pop))
  (import "env" "__exc_active" (func $imp___exc_active (result i32)))
  (import "env" "__exc_type" (func $imp___exc_type (result i32)))
  (import "env" "__exc_throw" (func $imp___exc_throw (param i32 i32)))
  (import "env" "__exc_clear" (func $imp___exc_clear))
  (import "env" "__exc_matches" (func $imp___exc_matches (param i32 i32) (result i32)))
  (import "env" "__free" (func $imp___free (param i32)))
  (import "env" "printf" (func $imp_printf (param f64 f64 f64 f64 f64 f64 f64 f64) (result i32)))

  (memory $mem 1)

  (table $fn_table 4 funcref)

  ;; global __frame_ptr
  (global $__frame_ptr (mut i32) (i32.const 0))

  ;; global __stack_ptr
  (global $__stack_ptr (mut i32) (i32.const 1024))

  (data (i32.const 16) "%s\00")
  (data (i32.const 20) "\0a\00")
  (data (i32.const 24) "The square of 5 is \00")
  (data (i32.const 44) "%d\00")
  (data (i32.const 48) ".\0a\00")
  (data (i32.const 52) "%c\00")

  (elem (table $fn_table) (i32.const 0) func $display_message__pv $square__i $main $display_message__c)

  ;; function display_message__pv
  (func $display_message__pv (param $text i32)
    (local $__frame i32)
    (local $__parent_frame i32)
    global.get $__frame_ptr
    local.set $__parent_frame
    global.get $__stack_ptr
    local.set $__frame
    local.get $__frame
    global.set $__frame_ptr
    global.get $__stack_ptr
    i32.const 8
    i32.add
    global.set $__stack_ptr
    local.get $__frame
    i32.const 0
    i32.add
    local.get $text
    i32.store
    i32.const 16
    f64.convert_i32_s
    local.get $__frame
    i32.const 0
    i32.add
    i32.load
    f64.convert_i32_s
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    call $imp_printf
    drop
    i32.const 20
    f64.convert_i32_s
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    call $imp_printf
    drop
  )

  ;; function square__i
  (func $square__i (param $x i32) (result i32)
    local.get $x
    local.get $x
    i32.mul
    return
    i32.const 0
    return
  )

  ;; function main
  (func $main (result i32)
    (local $__frame i32)
    (local $msg i32)
    (local $__parent_frame i32)
    global.get $__frame_ptr
    local.set $__parent_frame
    global.get $__stack_ptr
    local.set $__frame
    local.get $__frame
    global.set $__frame_ptr
    global.get $__stack_ptr
    i32.const 16
    i32.add
    global.set $__stack_ptr
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 72
    i32.store8
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 1
    i32.add
    i32.const 101
    i32.store8
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 2
    i32.add
    i32.const 108
    i32.store8
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 3
    i32.add
    i32.const 108
    i32.store8
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 4
    i32.add
    i32.const 111
    i32.store8
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 5
    i32.add
    i32.const 32
    i32.store8
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 6
    i32.add
    i32.const 87
    i32.store8
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 7
    i32.add
    i32.const 111
    i32.store8
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 8
    i32.add
    i32.const 114
    i32.store8
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 9
    i32.add
    i32.const 108
    i32.store8
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 10
    i32.add
    i32.const 100
    i32.store8
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 11
    i32.add
    i32.const 33
    i32.store8
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 12
    i32.add
    i32.const 0
    i32.store8
    local.get $__frame
    i32.const 0
    i32.add
    call $display_message__pv
    i32.const 24
    f64.convert_i32_s
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    call $imp_printf
    drop
    i32.const 44
    f64.convert_i32_s
    i32.const 5
    call $square__i
    f64.convert_i32_s
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    call $imp_printf
    drop
    i32.const 48
    f64.convert_i32_s
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    call $imp_printf
    drop
    i32.const 0
    local.get $__parent_frame
    global.set $__frame_ptr
    local.get $__frame
    global.set $__stack_ptr
    return
    i32.const 0
    local.get $__parent_frame
    global.set $__frame_ptr
    local.get $__frame
    global.set $__stack_ptr
    return
  )

  ;; function display_message__c
  (func $display_message__c (param $text i32)
    i32.const 52
    f64.convert_i32_s
    local.get $text
    f64.convert_i32_s
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    call $imp_printf
    drop
    i32.const 20
    f64.convert_i32_s
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    call $imp_printf
    drop
  )

  (export "display_message__pv" (func $display_message__pv))
  (export "square__i" (func $square__i))
  (export "main" (func $main))
  (export "display_message__c" (func $display_message__c))
  (export "memory" (memory $mem))
  (export "__frame_ptr" (global $__frame_ptr))
  (export "__stack_ptr" (global $__stack_ptr))
)